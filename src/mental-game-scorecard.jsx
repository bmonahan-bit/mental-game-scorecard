...
export default function App() {
  ...
  // ─── Global layout setup: lock body scroll, prevent iOS pinch/auto-zoom,
  //     and guarantee inputs never trigger the 16px zoom. Runs once on mount.
  React.useEffect(()=>{
    // 1. Viewport meta — prevent pinch-zoom and input-focus zoom.
    //    All inputs in the app are already ≥16px, so disabling user-scaling
    //    is safe and stops the "app zooms in and stays zoomed" bug.
    ...
    // 2. Lock html/body so ONLY the in-shell scroll containers scroll.
    //    Without this, content overflowing the 100svh shell scrolls the body.
    const styleEl = document.createElement("style");
    styleEl.setAttribute("data-mgs-global", "1");
    styleEl.textContent = `
      html, body { height: 100%; overflow: hidden; overscroll-behavior: none; }
      body { position: fixed; inset: 0; width: 100%; }
      /* Failsafe: no input should trigger iOS auto-zoom on focus. */
...
function PaywallView({onUnlock, onBack, P, S}) {
  ...
  return (
    <div style={{height:"100svh",display:"flex",flexDirection:"column",background:P.bg,maxWidth:480,margin:"0 auto",overflow:"hidden",fontFamily:"'Avenir Next','SF Pro Display',-apple-system,sans-serif"}}>
      {/* Header */}
      <div style={{background:dm?`linear-gradient(175deg,${PM_NAVY} 0%,#141416 60%,#1c1c1f 100%)`:`linear-gradient(175deg,${PM_NAVY} 0%,#2a3020 100%)`,borderBottom:`2px solid ${PM_GOLD}44`,padding:"calc(16px + env(safe-area-inset-top,0px)) 20px 20px",flexShrink:0,position:"relative",overflow:"hidden"}}>
